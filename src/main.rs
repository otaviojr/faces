use std::{sync::Mutex, fs::{self, DirEntry}, str::FromStr, error::Error, io::Write};
use image::{GenericImage, EncodableLayout, GenericImageView, imageops};
use rand::thread_rng;
use rand::seq::SliceRandom;
use neuron::{Neuron, math::Tensor, layers::{LinearLayer, LinearLayerConfig, ConvLayer, ConvLayerConfig, FlattenLayer, PoolingLayer, PoolingLayerConfig}, activations::{ReLU, Sigmoid, Tanh, SoftMax}, cost::Functions};
...
*/
    ...
    */
    ...
    neuron.add_layer(Box::new(Mutex::new(ConvLayer::new(3, 14, (5,5), ConvLayerConfig { activation: Box::new(ReLU::new()), learn_rate: 10e-3, padding: 0, stride: 1, weights_low: -1.5 * (10.0 as f64).exp(), weights_high: 1.5 * (-10.0 as f64).exp() }))));
    neuron.add_layer(Box::new(Mutex::new(PoolingLayer::new((2,2), PoolingLayerConfig { stride: 2}))));
    //neuron.add_layer(Box::new(Mutex::new(ConvLayer::new(14, 36, (5,5), ConvLayerConfig { activation: Box::new(ReLU::new()), learn_rate: 10e-6, padding: 0, stride: 1, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high:  1.5 * (-10.0 as f64).exp()}))));
    //neuron.add_layer(Box::new(Mutex::new(PoolingLayer::new((2,2), PoolingLayerConfig { stride: 2 }))));
    //neuron.add_layer(Box::new(Mutex::new(ConvLayer::new(36, 36, (3,3), ConvLayerConfig { activation: Box::new(ReLU::new()), learn_rate: 10e-7, padding: 0, stride: 1, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high:  1.5 * (-10.0 as f64).exp()}))));
    //neuron.add_layer(Box::new(Mutex::new(PoolingLayer::new((2,2), PoolingLayerConfig { stride: 1 }))));
    //neuron.add_layer(Box::new(Mutex::new(ConvLayer::new(36, 16, (3,3), ConvLayerConfig { activation: Box::new(ReLU::new()), learn_rate: 10e-7, padding: 0, stride: 1, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high:  1.5 * (-10.0 as f64).exp()}))));
    //neuron.add_layer(Box::new(Mutex::new(PoolingLayer::new((2,2), PoolingLayerConfig { stride: 2 }))));
    neuron.add_layer(Box::new(Mutex::new(FlattenLayer::new())));
    neuron.add_layer(Box::new(Mutex::new(LinearLayer::new(1400, 1400, LinearLayerConfig{ activation: Box::new(ReLU::new()), learn_rate: 10e-3, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high: 1.5 * (-10.0 as f64).exp()}))));
    //neuron.add_layer(Box::new(Mutex::new(LinearLayer::new(144, 64, LinearLayerConfig{ activation: Box::new(ReLU::new()), learn_rate: 10e-8, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high: 1.5 * (-10.0 as f64).exp()}))));
    neuron.add_layer(Box::new(Mutex::new(LinearLayer::new(1400, 2, LinearLayerConfig{ activation: Box::new(SoftMax::new()), learn_rate: 10e-3, weights_low: -1.5 * (-10.0 as f64).exp(), weights_high: 1.5 * (-10.0 as f64).exp()}))));
    ...
    for path in files.iter() {
        ...
        img = img.resize_to_fill((img_size*10) as u32, (img_size*10) as u32, imageops::FilterType::Nearest);
